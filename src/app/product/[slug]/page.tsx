...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen relative overflow-hidden bg-[#020617] selection:bg-emerald-500/30">
      <CustomCursor />
      <Navbar />
      ...
      <div className="absolute inset-0 w-full h-full pointer-events-none overflow-hidden">
        <motion.div 
          ...
          className="absolute top-[-5%] left-[-10%] w-[50rem] h-[50rem] bg-emerald-900/10 rounded-full blur-[150px] mix-blend-screen" 
        />
        <div className="absolute bottom-[20%] right-[-10%] w-[40rem] h-[40rem] bg-emerald-600/5 rounded-full blur-[120px] mix-blend-screen" />
        <div className="absolute top-[30%] right-[10%] w-[30rem] h-[30rem] bg-cyan-900/10 rounded-full blur-[130px] mix-blend-screen" />
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-start">
            ...
            <div className="space-y-12">
              ...
              <motion.div 
                ...
                className="glass-premium p-8 sm:p-12 rounded-[3rem] shadow-2xl relative overflow-hidden group border-white/5"
              >
                {/* Premium Accent */}
                ...
                <div className="space-y-8">
                  ...
                  <div className="flex items-end gap-5">
                    ...
                    {product.regular_price && product.price && product.regular_price !== product.price && (
                      <div className="text-2xl text-slate-600 line-through opacity-50 mb-2 font-bold">
                        {product.regular_price} DZD
                      </div>
                    )}
                  </div>
                  
                  <div className="w-full h-px bg-gradient-to-r from-emerald-500/30 via-white/5 to-transparent" />
                  
                  <div 
                    className="prose prose-invert prose-emerald max-w-none text-slate-400 leading-relaxed text-xl font-medium"
                    ...
                    {[
                      ...
                      <div key={i} className="flex flex-col items-center text-center gap-4 group/badge">
                        ...
                        <div className="space-y-1">
                          ...
                          <span className="block text-[11px] font-black uppercase tracking-widest text-slate-500 group-hover/badge:text-white transition-colors">{badge.text}</span>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <footer className="relative z-10 py-20 px-6 border-t border-white/5 bg-black/40 backdrop-blur-2xl">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-10">
            ...
            <div className="flex gap-8">
              {['سياسة الخصوصية', 'شروط الاستخدام', 'سياسة الاسترجاع'].map((link) => (
                <a key={link} href="#" className="text-[11px] font-bold uppercase tracking-widest text-slate-500 hover:text-emerald-400 transition-colors">{link}</a>
              ))}
            </div>
